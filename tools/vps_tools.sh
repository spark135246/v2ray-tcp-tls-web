...
setSwap() {
  ${sudoCmd} ${systemPackage} install curl -y -qq
  curl -sL https://raw.githubusercontent.com/spark135246/v2ray-tcp-tls-web/master/tools/set_swap.sh | bash
}

rmAliyundun() {
  ${sudoCmd} ${systemPackage} install curl -y -qq
  curl -sL https://raw.githubusercontent.com/spark135246/v2ray-tcp-tls-web/master/tools/rm_aliyundun.sh | bash
}
